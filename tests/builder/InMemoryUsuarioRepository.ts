...
export class InMemoryUsuarioRepository implements IUsuarioRepository {
    ...
    async listByRole(role: string): Promise<Usuario[]> {
        return this.usuarios.filter(u => {
            if (role === 'bolsista' || role === 'scholarship_holder') return !!u.bolsista;
    ...
    async atribuirPapel(userId: string, dto: { papel: string; acao: 'atribuir' | 'remover'; dados?: any }): Promise<void> {
        const usuario = this.usuarios.find(u => u.id === userId);
        if (!usuario) return;

        if (dto.acao === 'remover') {
            if (dto.papel === 'coordenador') usuario.coordenador = undefined;
            if (dto.papel === 'tutor') usuario.tutor = undefined;
            if (dto.papel === 'bolsista') usuario.bolsista = undefined;
            return;
        }
